...
sys.path.append(str(Path(__file__).parent))  # aponta para /datasets
...
class PbfMapDataset(SpatialDataset):
    def __init__(
        ...
        self.type_key_str = (
            type_key_str  # key name of the class type in the input data dictionary
        )

        super(PbfMapDataset, self).__init__(
            self.tokenizer, max_token_len, distance_norm_factor, sep_between_neighbors
        )
    ...
    def load_data(self, index):
        ...
        line = self.data[
            index
        ]  # take one line from the input data according to the index
        ...
        if self.random_remove_neighbor != 0:
            ...
            neighbor_keep_arr = (
                rand_neighbor >= self.random_remove_neighbor
            )  # select the neighbors to be removed
        ...
        train_data = self.parse_spatial_context(
            pivot_name,
            pivot_pos,
            neighbor_name_list,
        ...
        if self.with_type:
            ...
            train_data["pivot_type"] = torch.tensor(
...
class PbfMapDatasetMarginRanking(SpatialDataset):
    def __init__(
        ...
        super(PbfMapDatasetMarginRanking, self).__init__(
            self.tokenizer, max_token_len, distance_norm_factor, sep_between_neighbors
        )
    ...
    def prepare_all_types_data(self):
        ...
        for type_name in type_list:
            ...
            type_data = self.parse_spatial_context(
                type_name,
                type_pos,
                pivot_dist_fill=0.0,
                neighbor_name_list=[],
    ...
    def load_data(self, index):
        ...
        if self.mode == "train":
            ...
            postive_type_data = self.parse_spatial_context(
                postive_name,
                positive_pos,
                pivot_dist_fill=0.0,
                neighbor_name_list=[],
            ...
            negative_type_data = self.parse_spatial_context(
                negative_name,
                negative_pos,
                pivot_dist_fill=0.0,
                neighbor_name_list=[],
