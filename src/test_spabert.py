...
def testing(args):
    ...
    if args.num_classes == 9:
        london_file_path = "../data/test_data/osm-point-london-typing.json"
        california_file_path = "../data/test_data/osm-point-california-typing.json"
        TYPE_LIST = CLASS_9_LIST
        type_key_str = "class"
    elif args.num_classes == 74:
        london_file_path = (
            "../../semantic_typing/data/sql_output/osm-point-london-typing-ranking.json"
        )
    ...
    if bert_option == "bert-base":
        ...
        config = SpatialBertConfig(
            use_spatial_distance_embedding=not if_no_spatial_distance,
            num_semantic_types=len(TYPE_LIST),
        )
    ...
    london_dataset = PbfMapDataset(
        data_file_path=london_file_path,
        tokenizer=tokenizer,
    ...
    california_dataset = PbfMapDataset(
        data_file_path=california_file_path,
        tokenizer=tokenizer,
        max_token_len=max_token_len,
        distance_norm_factor=distance_norm_factor,
        spatial_dist_fill=spatial_dist_fill,
    ...
    for batch in loop:
        ...
        mrr = label_ranking_average_precision_score(
            onehot_labels.cpu().detach().numpy(), outputs.logits.cpu().detach().numpy()
        )
    ...
    precisions, recalls, fscores, supports = precision_recall_fscore_support(
        np.argmax(np.array(gt_list), axis=1),
...
def main():
    ...
    if args.prediction_save_dir is not None and not os.path.isdir(
        args.prediction_save_dir
    ):
