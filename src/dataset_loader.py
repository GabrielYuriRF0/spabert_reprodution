...
class SpatialDataset(Dataset):
    ...
    def parse_spatial_context(
        self,
        pivot_name,
        pivot_pos,
        neighbor_name_list,
        neighbor_geometry_list,
        spatial_dist_fill,
        pivot_dist_fill=0,
    ):
        ...
        # process pivot
        pivot_name_tokens = self.tokenizer.convert_tokens_to_ids(
            self.tokenizer.tokenize(pivot_name)
        )
        ...
        )  # random number for masking entities including neighbors and pivot
        ...
        for neighbor_name, neighbor_geometry, rnd in zip(
            neighbor_name_list, neighbor_geometry_list, rand_entity[1:]
        ):
            if not neighbor_name[0].isalpha():
                # only consider neighbors starting with letters
                continue

            neighbor_token = self.tokenizer.convert_tokens_to_ids(
                self.tokenizer.tokenize(neighbor_name)
            )
            ...
            if (
                "coordinates" in neighbor_geometry
            ):  # to handle different json dict structures
                neighbor_lng_list.extend(
                    [
                        (neighbor_geometry["coordinates"][0] - pivot_lng)
                        / self.distance_norm_factor
                    ]
                    * neighbor_token_len
                )
                neighbor_lat_list.extend(
                    [
                        (neighbor_geometry["coordinates"][1] - pivot_lat)
                        / self.distance_norm_factor
                    ]
                    * neighbor_token_len
                )
                ...
                neighbor_lng_list.extend(
                    [(neighbor_geometry[0] - pivot_lng) / self.distance_norm_factor]
                    * neighbor_token_len
                )
                neighbor_lat_list.extend(
                    [(neighbor_geometry[1] - pivot_lat) / self.distance_norm_factor]
                    * neighbor_token_len
                )
                neighbor_token_list.extend(neighbor_token)
        ...
        if sent_len > max_token_len_middle:
            ...
            dist_lat_list = (
                [spatial_dist_fill]
                + dist_lat_list[:max_token_len_middle]
                + [spatial_dist_fill]
            )
            dist_lng_list = (
                [spatial_dist_fill]
                + dist_lng_list[:max_token_len_middle]
                + [spatial_dist_fill]
            )
            attention_mask = (
                [False] + [1] * max_token_len_middle + [False]
            )  # make sure SEP and CLS are not attented to
        else:
        ...
        masked_entity_input = [
            mask_token_id if i in entity_mask_indices else pseudo_sentence[i]
        ...
        token_mask_arr = (
            (rand_token < 0.15)
            & (np.array(pseudo_sentence) != cls_token_id)
            & (np.array(pseudo_sentence) != sep_token_id)
            & (np.array(pseudo_sentence) != pad_token_id)
        )
        token_mask_indices = np.where(token_mask_arr)[0]

        masked_token_input = [
            mask_token_id if i in token_mask_indices else pseudo_sentence[i]
            for i in range(0, max_token_len)
        ]
        ...
        train_data["sent_position_ids"] = torch.tensor(
            np.arange(0, len(pseudo_sentence))
        )
